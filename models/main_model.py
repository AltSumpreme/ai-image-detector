...
class MultiSignalDeepfakeDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_branch_outputs: bool = False) -> dict[str, torch.Tensor]:
        ...
        out = {"logits": logits, "probability": prob}
        if return_branch_outputs:
            out["branches"] = {"srm": srm, "fft": fft, "dwt": dwt, "vit": vit, "physical": phys}
        return out
